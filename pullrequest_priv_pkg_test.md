### PROMPT
```
Write unit tests for the decodePullRequests(), decodePullRequestsList(), decodePullRequestsComments(), and decodePullRequestsCommentsLis() functions. Include both success and failure cases using valid and invalid inputs.
```


I'll create comprehensive unit tests for the four decode functions. Let me first examine the existing code structure to understand what needs to be tested.Now I'll create comprehensive unit tests for the four decode functions. Based on the code, I can see these functions decode API responses into structured Go types. Let me create a new test file with both success and failure cases.
...
func TestDecodePullRequests_ComplexStructure(t *testing.T) {
	...
	input := map[string]interface{}{
		...
		"draft":                 false,
		"close_source_branch":   true,
		"created_on":            createdTime.Format(time.RFC3339),
		"updated_on":            updatedTime.Format(time.RFC3339),
		"author": map[string]interface{}{
...
## Success Cases:
1. **decodePullRequests**: Valid data with all fields, minimal data, complex nested structures
2. **decodePullRequestsList**: Valid list with multiple items, empty list, missing pagination fields
3. **decodePullRequestsComments**: Valid comment with content and parent, comment without parent, minimal data
4. **decodePullRequestsCommentsList**: Valid list with multiple comments, empty list, missing pagination fields

## Failure Cases:
1. **Error type responses**: All decode functions handle API error responses
2. **Invalid formats**: Non-map inputs that should return "Not a valid format" errors
3. **Invalid field types**: Testing when pagination fields have wrong types (defaults to 0)
4. **Invalid time formats**: Testing time parsing issues
5. **Mixed valid/invalid items**: Lists containing some error items (should be skipped)

The tests use the `testify/assert` package (which is already used in the existing test file) for clean assertions and cover both happy paths and edge cases.